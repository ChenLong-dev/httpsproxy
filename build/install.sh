...
SSLPATH=${SHELLPATH}/ssl
DSTSSLPATH=/saasdata/${TARGET}/grpc/ssl
APPVERPATH=${INSTALLPATH}/app
...
function install_service() {
    ...
    # 安装版本信息文件
    if [ ! -d ${APPVERPATH} ]; then
        mkdir -p ${APPVERPATH}
    else
        if [ -f ${APPVERPATH}/${APPVERSION} ]; then
            mv ${APPVERPATH}/${APPVERSION} ${APPVERPATH}/${APPVERSION}_bak_${VERSION}
        fi
    fi
    cp ${SHELLPATH}/${APPVERSION} ${APPVERPATH}/${APPVERSION}
    if [ $? != 0 ]; then
        return 1
    fi

    cat ${SHELLPATH}/${APPVERSION} >> ${APPVERPATH}/${UPDHISTORY}
    echo $UPDATETIME >> ${APPVERPATH}/${UPDHISTORY}

    md5sum ${INSTALLFILE} ${APPVERPATH}/${APPVERSION}
    cat ${APPVERPATH}/${APPVERSION}
    echo
...
function install_ssl() {
     if [ ! -d ${DSTSSLPATH} ]; then
        mkdir -p ${DSTSSLPATH}
     fi
     cp ${SSLPATH}/* ${DSTSSLPATH}
     md5sum ${DSTSSLPATH}/*
}
...
function print_help {
    ...
    echo -e "\033[35m        -z         : 安装ssl相关文件 \033[0m"
